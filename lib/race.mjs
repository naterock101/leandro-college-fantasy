...
export function raceFrames(byWeek, results, managers = []) {
  ...
  const opening = blank();
  for (const m of all) if (Object.hasOwn(first, m)) opening[m] = 0;
  if (perGame) frames.push({ x: 0, week: null, totals: opening });
  ...
  weeks.forEach((w, wi) => {
    ...
    games.forEach((g, gi) => {
      ...
      if (!m || !g.points || running[m] === null || running[m] === undefined) return;
      running = { ...running, [m]: running[m] + g.points };
      frames.push({ x: w.x0 + ((gi + 1) / n) * (w.x1 - w.x0), week: null, totals: running });
    });
    ...
    const closed = blank();
    for (const m of all) if (Object.hasOwn(cum, m)) closed[m] = cum[m].points;
    running = { ...closed };
    ...
    if (last && Math.abs(last.x - w.x1) < 1e-9) {
      last.totals = closed;
      last.week = w;
    } else {
      frames.push({ x: w.x1, week: w, totals: closed });
    }
  });
...
/**
 * Where the x window starts, as a fraction of the season.
 *
...
export function windowStart(weekCount, lastN) {
  ...
  return span(weekCount - lastN, weekCount);
}
