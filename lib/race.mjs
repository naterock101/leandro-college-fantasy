...
export function raceFrames(byWeek, results, managers = []) {
  ...
  weeks.forEach((w, wi) => {
    ...
    /* A week with games in it is a span and closes on its right-hand edge; a
       ...
       Keyed on this week rather than on the season: `perGame` is true as soon
       as any one week has games, and a season is routinely a mix - the bot
       writes a week into `byWeek` before `results` catches up, and the golden
       fixture's two postseason weeks carry no games at all. Those are exactly
       the weeks that were still being drawn off their own labels. */
    const at = games.length ? w.x1 : w.at;
