...
export function raceFrames(byWeek, results, managers = []) {
  ...
  weeks.forEach((w, wi) => {
    ...
    /* A week with games in it is a span and closes on its right-hand edge; a
       ...
       the next. The label is always the middle, so getting this wrong put
       every fallback dot half a week to the right of its own name. */
    const at = perGame ? w.x1 : w.at;
