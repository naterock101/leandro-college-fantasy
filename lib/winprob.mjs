 ...
 * and the same subtraction done in wins.
 *
 ...
 *
 * The record comes from the same ledger rather than from a second pass,
 * because the thing a reader compares an expected 9.8-4.2 against is the
 * actual record *over those same games* - and the actual record in the
 * standings is over every settled game, priced or not. Computing the two
 * halves anywhere but here is how they would come to cover different seasons.
 * `losses` is published rather than left as `games - wins` for the same reason
 * `delta` is: the page should be printing numbers, not deriving them, and
 * `expectedLosses` genuinely cannot be derived on screen - subtracting a
 * rounded 9.8 from 14 in binary floating point gives 4.199999999999999.
 *
...
export function luckOf(outcomes) {
  let actual = 0, expected = 0, wins = 0, expectedWins = 0;
  for (const o of outcomes) {
    if (o.won) { actual += o.value; wins += 1; }
    expected += o.probability * o.value;
    /* A win is a win whatever the team is worth, so the record weights every
       game the same - which is exactly what makes it a different sentence
       from the points beside it. A manager can be under on expected wins and
       over on expected points by losing cheap games and winning dear ones. */
    expectedWins += o.probability;
  }
  const e = round1(expected);
  const w = round1(expectedWins);
  return {
    games: outcomes.length, actual, expected: e, delta: round1(actual - e),
    wins, losses: outcomes.length - wins,
    expectedWins: w, expectedLosses: round1(outcomes.length - w),
  };
}
