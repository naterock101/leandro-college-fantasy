 ...
 *
 * @typedef {{spread: number | null, favorite: string | null,
 *             probability?: number} | null | undefined} Line
 */
 ...
 *
 * **Corroborated on a second, larger, newer sample.** Phil Steele's tabulation
 * of outright underdog wins covers 20,505 college games across 1997-2023 and
 * was fitted independently: it gives **14.52**, and 14.4 is the point of least
 * weighted error on his bands too (1.53% against 16's 2.38%). Two samples,
 * different decades, different authors, different framing - one counts
 * favourites winning and the other counts underdogs winning - landing on 14.43
 * and 14.52 is about as much as this kind of number can be pinned down.
 *
 * **A known bias, which is not noise.** The model overstates short favourites.
 * On Stassen the 0.5-3.5 band comes in 2.8 points below the model; on Steele
 * the "+3 or less" band is 3.6 points below (47.5% of those underdogs won
 * outright, against the 43.9% the model expects). This comment previously
 * called that a fault in Stassen's tabulation, on the strength of three
 * adjacent buckets there that cannot all be right - 1.5-point favourites at
 * 45.0%, 2-point at 64.1%, 2.5-point at 45.2%. Those buckets are still
 * suspect, but a second source built a different way shows the same effect in
 * the same direction, so the band-level effect is real: a game priced inside a
 * field goal is closer to a coin flip than a normal centred on the spread
 * makes it.
 *
 * It is **deliberately not corrected**. Fitting a second parameter to two
 * band-level observations is an epicycle with nothing left over to validate it
 * against, and the cost of leaving it is small: short prices are about 15% of
 * games, so over a manager's season the bias is worth on the order of a tenth
 * of an expected win. It is documented instead, which is the honest trade.
 *
 * Otherwise the shape holds, and that is the part worth checking, because a
 * sigma can be fitted to any monotone curve while describing the wrong
 * distribution. Above a field goal every band on both sources lands within
 * noise of the model. Separately, Boyd's Bets finds essentially no correlation
 * between the size of the spread and the size of the scatter in college
 * football - 0.09 - which is the assumption a single sigma makes.
 *
...
/**
 * The favourite's chance, from whichever half of the price the books gave us.
 *
 * A stored `probability` is preferred over the spread, and there is no case
 * where a price carries both: it is written by the moneyline fallback in
 * lib/lines.mjs, for games a book would not put a number on. Preferring it is
 * not a tie-break but a shortcut past the model - a moneyline *is* a
 * probability, where a spread has to be turned into one by assuming how
 * results scatter. Sigma is the estimate here, so skipping it is strictly less
 * guessing.
 *
 * `null` for a game with no line at all.
 *
 * @param {Line} line
 * @returns {number | null}
 */
export function favouriteChance(line) {
  if (!line) return null;
  if (typeof line.probability === "number") return line.probability;
  if (typeof line.spread !== "number") return null;
  return favouriteProbability(line.spread);
}
