 ...
 *
 * `floor` exists because the awards fail in three different ways. A chance is a
 * property of a game that happened and is always a real number, so any
 * candidate at all is a real one. A weekly total, a luck delta and a count are
 * sums over a season that may not have started: every manager on nought points
 * is not an eight-way tie for Best Week, it is a trophy nobody has won. And a
 * cover is a property of a game that happened which can still be negative -
 * nobody wins a blowout for losing to the number by the least.
 */
const DEFS = [
  {
  ...
  },
  {
    ...
    blurb: "Beat the closing spread by the most",
    unit: "margin", floor: 0,
    /* Against the number, not against nought. A 40-point favourite scraping
       home is a bad Saturday however large the scoreboard says the margin was,
       and a touchdown underdog winning by three touchdowns is the best result
       of the week on half the margin. `expectedMargin` is what the line
       expected of the side that won, so the subtraction is the cover.

       The cost is that a game the books never priced wins nothing here, where
       under a raw margin it could. That is paid knowingly: a sixth of the live
       season carries no line and one of those could be the season's biggest
       hiding. The alternative is a trophy that ranks a cover against a raw
       margin, which is two quantities under one heading, and the reader cannot
       tell which one they are looking at.

       A floor, unlike the other two price-based awards, and the move to a
       cover is what put it there. A chance is a real number for any game that
       was played, so any candidate there is a real one; a cover can be
       negative, and with no floor a week in which no rostered favourite
       covered handed the trophy to whoever missed by least - a card reading
       "Blowout: -7 on the line". A push is not beating the number either, so
       the floor is strict. */
    candidates: (rows) => rows.flatMap((r) => {
      const m = marginOf(r.score);
      if (!r.winner.manager || m === null || typeof r.expectedMargin !== "number") return [];
      const cover = m - r.expectedMargin;
      return [{ manager: r.winner.manager, sort: cover, value: cover,
                /* Both numbers, because a cover is a subtraction and a card
                   showing one of the two asks the reader to trust the other. */
                detail: `${r.winner.team} ${r.score} over ${r.loser.team}`
                        + (r.line ? ` · ${r.line}` : ""),
                row: r }];
    }),
  },
