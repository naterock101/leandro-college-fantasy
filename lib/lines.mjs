...
export function mergeLines(stored, observations) {
  ...
  for (const obs of observations) {
    ...
    /* A spread outranks a moneyline, across runs as well as within one. The
       sequence this is for is real and one-way: a book posts a spread, then
       pulls it as kickoff nears and leaves only a moneyline up. Taking the
       downgrade would swap a stored closing spread for a quote that says less,
       and on a settled game it would rewrite what the market said at the time
       - which is the same thing the merge-only rule exists to prevent. */
    if (obs.price.spread === null && typeof prev?.spread === "number") {
