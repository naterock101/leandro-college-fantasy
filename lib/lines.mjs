...
/**
 * The `dates` values covering a run: yesterday, today and tomorrow, Eastern.
 *
 ...
 *
 * Three days rather than the week, all of them earning their place:
 * - tomorrow, so a game is priced before kickoff even if the next baseline run
 *   is the last one before it,
 * - today, for the slate in progress,
 * - yesterday, which is the easy one to drop and the one that must not be. A
 ...
 *   is the last thing that can happen before ESPN deletes the odds.
 *
 * @param {Date} now
