...
export function fpiObservation(predictor, competition, id, seenAt) {
  ...
  /* Held short of a certainty. FPI says 99.8 on the sort of mismatch that
     reaches this function, and rounding that to whole percents makes it 100 -
     so this path, alone in the repo, could publish a team as certain to win.
     `percent` refuses to print 100 for exactly that reason and the spread
     model tops out at 0.9975; this is the same refusal at the source, where
     the projection reads it too. */
  const chance = Math.min(Math.max(p, 1 - p), 0.99);
  return {
    id: String(id),
    seenAt,
